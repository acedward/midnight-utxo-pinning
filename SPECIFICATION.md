...
| R-24 | The validity bound of a candidate MUST be the minimum of the root retention window of the deployed ledger (shielded and Dust inputs) and the smallest intent TTL (unshielded inputs, Dust spends); it MUST be read from the network, not assumed, and MUST be returned with the candidate. | 2, candidates |
